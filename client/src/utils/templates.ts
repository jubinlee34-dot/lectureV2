 ...
 *
 * 결과보고서: 공공기관·복지관·학교 제출용 행정 보고서 형식 (금지어 필터링 적용)
 * 블로그 초안: 강사 현장 후기 형식 (스토리텔링 및 SEO 중심, H1 단일 해시태그 규격)
 */
...
/**
 * 교육 진행 흐름 중심 서사형 문장을 생성합니다. (단순 목록 금지 규정 적용)
 */
function buildContentNarrative(lecture: Lecture): string {
  const cleanLines = lecture.content
    .split("\n")
    .map((l) => l.trim().replace(/^[-*•\d.]+\s*/, "")) // 불릿 및 숫자 기호 제거
    .filter(Boolean);

  if (cleanLines.length === 0) {
    return `교육은 ${lecture.topic}에 관한 기초 개념 설명과 주요 도구 소개로 시작하여 개별 실습을 거쳐 질의응답 및 개별 피드백 순으로 차례대로 조율하며 진행되었다.`;
  }

  const firstStep = cleanLines[0];
  const subsequentSteps = cleanLines.slice(1);
...
/**
 * 참여자 반응 및 교육 성과 섹션을 생성합니다. (소감, 반응, 질문, 메모 유기적 결합)
 */
function buildParticipantSection(lecture: Lecture): string {
  const parts: string[] = [];

  const hasReaction = lecture.participantReaction?.trim();
  const hasQuestion = lecture.memorableQuestion?.trim();
  const hasMemo = lecture.instructorMemo?.trim();

  if (hasReaction) {
    parts.push(`교육 진행 과정에서 관찰된 참여자들의 반응과 분위기는 매우 고무적이었다. 구체적으로는 ${hasReaction}`);
  } else {
    parts.push(`참여자들은 교육 초반 낯선 기능에 대한 기기 조작을 일부 낯설어하였으나, 강사 피드백과 실습이 반복됨에 따라 높은 주의집중 상태를 유지하며 적극적인 실습 의지를 보여주었다.`);
  }

  if (hasQuestion) {
    parts.push(`특히 병원 예약 등 실무 활용 단계에서 한 참여자가 "${hasQuestion}"와 같은 질문을 제기하였으며, 이는 교육 내용을 실무 및 실생활과 연계하려는 적극적인 학습 동기를 보여주었다.`);
  }

  if (hasMemo) {
    parts.push(`강사 관찰 기록에 의하면, ${hasMemo} 이 부분이 교육 완성도를 한층 올리는 핵심 요소로 작용하였다.`);
  }
...
function buildSummarySection(lecture: Lecture): string {
  ...
  const hasReflection = lecture.reflection?.trim();
  if (hasReflection) {
    parts.push(`현장 조율 결과 및 한계점 극복 과정으로서, ${hasReflection}`);
  } else {
    parts.push(`한계점으로는 수강생 간 개인별 이해 속도 및 기기 활용 수준의 차이로 인해 진행 속도의 유연한 조율이 수시로 요구되었으며, 차기 교육 구성 시 난이도별 분반 편성을 검토하거나 보조 강사 인력이 배치된다면 더욱 완성도 높은 수업이 될 것으로 예상된다.`);
  }
...
export function generateReport(lecture: Lecture): string {
  ...
  const background = buildBackground(lecture, orgType);
  const contentNarrative = buildContentNarrative(lecture);
  const participantSection = buildParticipantSection(lecture);
  const summarySection = buildSummarySection(lecture);
  const nextPlan = buildNextPlan(lecture, orgType);
...
function buildIntro(lecture: Lecture, orgType: ReturnType<typeof detectOrgType>): string {
  const { organization, target, topic, location } = lecture;
...
/**
 * 가장 반응이 좋았던 활동 섹션을 생성합니다. (실습 과정, 강사 관찰)
 */
function buildBestActivity(lecture: Lecture): string {
  ...
  const mainPoint = cleanLines.length > 0 ? cleanLines[0] : lecture.topic;
...
/**
 * 기억에 남는 순간 섹션을 생성합니다. (에피소드, 메모, 질문 반영)
 */
function buildMemorable(lecture: Lecture): string {
  const hasQuestion = lecture.memorableQuestion?.trim();
  const hasMemo = lecture.instructorMemo?.trim();

  if (hasQuestion || hasMemo) {
    let text = "";
    if (hasQuestion) {
      text += `중간에 한 참여자가 손을 번쩍 들며 "${hasQuestion}"라고 날카로운 의문을 던진 순간이 백미였다. 이 돌발 질문 하나 덕택에 다른 참여자들도 고개를 끄덕이며 수업 몰입도가 최고조에 달했다.\n\n`;
    }
    if (hasMemo) {
      text += `강사 가이드 측면에서도 매우 중요한 깨달음을 얻었다. ${hasMemo}`;
    }
    return text;
  }
...
function buildHashtags(lecture: Lecture): string {
  ...
  // 핵심 키워드
  tags.push(`#${lecture.topic.replace(/\s+/g, "")}`);
  tags.push(`#${lecture.title.replace(/\s+/g, "").substring(0, 12)}`);
  tags.push(`#${lecture.organization.replace(/\s+/g, "").substring(0, 10)}`);
...
export function generateBlogDraft(lecture: Lecture): string {
  const orgType = detectOrgType(lecture.organization);
  const seoTitle = buildSeoTitle(lecture, orgType);
  const intro = buildIntro(lecture, orgType);
  const fieldStory = buildFieldStory(lecture);
  const bestActivity = buildBestActivity(lecture);
  const memorable = buildMemorable(lecture);
  const closing = buildClosing(lecture);
  const hashtags = buildHashtags(lecture);
