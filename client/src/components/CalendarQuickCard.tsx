  ...
  return (
    ...
    >
      <div className="mb-2 flex items-start justify-between gap-2">
        ...
        <Badge
          variant="outline"
          className={`shrink-0 text-[10px] font-semibold ${statusBadgeClass[lecture.workflowStage]}`}
        >
      ...
      {(starredBeforeTasks.length > 0 || starredAfterTasks.length > 0) && (
        <div className="mb-2 space-y-1 rounded-md border border-border/50 bg-muted/50 p-2 text-[10px]">
          {starredBeforeTasks.length > 0 && (
            <TaskSummary
              label="강의 전 필수"
              texts={starredBeforeTasks.map(task => task.text)}
            />
          )}
          {starredAfterTasks.length > 0 && (
            <TaskSummary
              label="강의 후 필수"
              texts={starredAfterTasks.map(task => task.text)}
            />
          )}
        </div>
      )}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {lecture.workflowStage !== "after" && (
          <QuickAction onClick={() => onAction(lecture, "detail")}>
            상세보기
          </QuickAction>
        )}
        {lecture.workflowStage === "before" && (
          <>
            <QuickAction
              onClick={() => onAction(lecture, "tasks")}
            ...
            {onAfterRecord && (
              <QuickAction
                onClick={() => onAction(lecture, "after-record")}
                tone="amber"
                icon={<ClipboardCheck className="h-3.5 w-3.5" />}
              >
                {afterRecordLabel}
        ...
        {lecture.workflowStage === "after" && (
          <>
            ...
            <QuickAction
              onClick={() => onAction(lecture, "report")}
              icon={<FileText className="h-3.5 w-3.5" />}
            >
              결과보고서
            </QuickAction>
            <QuickAction onClick={() => onAction(lecture, "blog")}>
              홍보 블로그 작성
            </QuickAction>
            {onPromote && (
...
function TaskSummary({ label, texts }: { label: string; texts: string[] }) {
  return (
    <div className="flex items-start gap-1">
      ...
      <span className="shrink-0 font-bold text-amber-800 dark:text-amber-400">
        [{label}]:
      </span>
